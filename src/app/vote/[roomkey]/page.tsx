...
export default async function VotePage({ params }: { params: Promise<{ roomkey: string }> }) {
  ...
  const electionData=await fetch(`${process.env.HOST_ADDRESS}/api/Elections/?roomid=${roomkey}`).then((res) => res.json())
  if (!electionData) {
    notFound()
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <p className="text-lg text-center text-gray-700 mb-6">{electionData.election.ElectionDesc}</p>
